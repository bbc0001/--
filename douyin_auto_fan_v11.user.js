...
(function() {
    ...
    let scriptActive = false; // 脚本是否已启动
    let stats = {
        skipped: 0,
        aLevel: 0,
        sLevel: 0,
    ...
    let consecutiveNoVideoCount = 0;

    function init() {
        console.log('[抖音自动助手 v11.0] 脚本已加载');
        console.log('📌 请进入推荐页后点击"启动脚本"按钮');
        ...
        updateStatus('等待手动启动...');
        console.log('[抖音自动助手] 面板已加载，请进入推荐页后启动');
    }

    // 启动脚本
    function startScript() {
        if (scriptActive) {
            console.log('[抖音自动助手] 脚本已在运行');
            return;
        }
    ...
    function startMainLoop() {
        ...
        mainLoopInterval = setInterval(async () => {
            if (!scriptActive || isResting || isProcessing) return;
    ...
    function createControlPanel() {
        ...
        controlPanel.innerHTML = `
            <style>
                ...
                #douyin-autofan-panel .button-group {
                    display: flex;
                    gap: 8px;
                    margin-top: 8px;
                }
                ...
                #douyin-autofan-panel #start-btn {
                    background: #22c55e;
                    color: white;
                }
                #douyin-autofan-panel #start-btn:hover {
                    background: #16a34a;
                }
                #douyin-autofan-panel #stop-btn {
                    background: #ef4444;
                    color: white;
                    display: none;
                }
                #douyin-autofan-panel #stop-btn:hover {
                    background: #dc2626;
                }
            </style>
            <h3>🎬 抖音自动收藏助手 v11.0</h3>
            
            <div class="rule-box">
                <div class="rule-title">📌 使用说明</div>
                <div>请先进入<strong>推荐页</strong>，然后点击启动</div>
            </div>
            ...
            <div class="status">
                ...
                <span id="status-text">等待手动启动...</span>
            </div>
            
            <div class="timer-text" id="timer-text">未启动</div>
            <div class="timer-bar">
            ...
            <div class="button-group">
                <button id="start-btn">▶️ 启动脚本</button>
                <button id="stop-btn">⏹ 暂停脚本</button>
            </div>
        `;
        ...
        document.getElementById('stop-btn').addEventListener('click', pauseScript);
    }
