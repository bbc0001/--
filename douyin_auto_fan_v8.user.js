...
(function() {
    ...
    const CONFIG = {
        ...
        VIDEO_LOAD_WAIT_TIME: 5000, // 视频加载等待时间
        ...
        // 点赞数字稳定检测设置
        LIKES_STABILIZATION_COUNT: 3, // 连续相同的次数才算稳定
        LIKES_STABILIZATION_DELAY: 500 // 检测间隔（毫秒）
    };
    ...
    function init() {
        ...
        console.log('📌 性能优化：降低资源占用，确保稳定运行');
    ...
    async function waitForVideoLoad() {
        ...
        if (video && video.readyState > 0) {
            console.log('[抖音自动助手] ✅ 视频已加载');
            return true;
        }
        
        console.log('[抖音自动助手] ⚠️ 视频加载可能未完成，继续执行');
        return false;
    }
                ...
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    for (const node of mutation.addedNodes) {
                        if (node.nodeName === 'VIDEO' || (node.querySelector && node.querySelector('video'))) {
                            const video = getVideoElement();
                            if (video && video.src !== lastObservedSrc) {
                                lastObservedSrc = video.src;
                                lastVideoSrc = '';
                                debouncedProcess();
                            }
                            return;
    ...
    function startMonitoring() {
        ...
        const startProcessing = () => {
            setTimeout(processCurrentVideo, 3000);
            setTimeout(() => {
                scheduleNextScroll();
            }, 5000);
        };
